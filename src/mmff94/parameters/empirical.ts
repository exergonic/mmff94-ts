...
// no δ, no corrections). In one line: Part V's eq. (18) prints a δ
// shrinkage that Halgren's own reference implementation never applied —
// the paper and the reference disagree, the 753-molecule validation
// suite (generated by BatchMin) reflects the reference, and this
// library matches the reference implementation, not the literal
// published equation, on this point (docs/implementer-notes.md §4.1).
// The published tables are used as-is: r(O) = 0.72 (the CCL errata and
// Tinker's mmffcovrad agree). Scope: the plain form is validated on
// BOij = 1 single bonds only (the suite's one empirical bond); the
// paper's δ/BOij/hybridization reductions for multiple and aromatic
// bonds are NOT implemented.
//
// The angle rules: the θ₀ protocol's full-miss path never fires in the
// suite (the par's wildcards resolve everywhere, 753/753), but the
// eq. (20) k now also backs the k_a = 0 wildcard rows — 1,875 suite
// angles take the empirical k (the 2026-08-06 fix, born from the
// P-centered angles of the vinyl phosphine fixture; the suite still
// matches BatchMin per-term to ≤1e-4, so the reference uses the
// empirical k there too). Both paths are pinned by
  ...
  if (!found) {
    ...
    if (central_single) {
      if (
        ...
        // 3.795 — their deviation, quantified in
        // docs/vinyl-phosphine-divergence.md. Do not "fix" it toward the
        // reference value.
        let pi_bc = 0.15;
