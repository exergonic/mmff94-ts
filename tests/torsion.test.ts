...
describe('Torsion Energy', () => {
  ...
  it('calculates energy for H-C-C-H dihedral at tau = 0° (eclipsed)', () => {
    ...
    //
    // Geometry: both H atoms on the SAME side of the C-C axis gives τ = 0° (eclipsed).
    const rCC = 1.5;
    const rCH = 1.1;
  ...
  it('calculates energy for H-C-C-H dihedral at tau = 180° (anti)', () => {
    // tau = 180°: E = (0.284/2)*(1+cos(180)) + (-1.386/2)*(1+cos(180)) + (0.314/2)*(1+cos(540))
    //           = 0.142*0 + (-0.693)*0 + 0.157*0 = 0.0
    //
    // Geometry: H atoms on OPPOSITE sides of the C-C axis gives τ = 180° (anti).
    const rCC = 1.5;
    const rCH = 1.1;
  ...
  it('calculates energy for H-C-C-H dihedral at tau = 60° (gauche, staggered ethane)', () => {
    // tau = 60°:  E = (0.284/2)*(1+cos(60)) + (-1.386/2)*(1+cos(-60)) + (0.314/2)*(1+cos(180))
    //           = 0.142*1.5 + (-0.693)*1.5 + 0.157*0 = 0.213 - 1.0395 = -0.8265
    //
    // Geometry: second H rotated 60° about the C-C axis (x) from the first.
    const rCC = 1.5;
    const rCH = 1.1;
    ...
    const mol: TypedMolecule = {
      atoms: [
        ...
        { index: 1, element: 'C', x: 0, y: 0, z: 0 },
        { index: 2, element: 'C', x: rCC, y: 0, z: 0 },
        { index: 3, element: 'H', x: rCC, y: rCH * Math.cos(Math.PI / 3), z: rCH * Math.sin(Math.PI / 3) },
      ],
      bonds: [
        { atom1: 0, atom2: 1, bond_order: 1 },
        { atom1: 1, atom2: 2, bond_order: 1 },
    ...
    expect(calc_torsion_energy(mol)).toBeCloseTo(-0.8265, 3);
  });
});
